...
function validateAndCollect(program) {
  ...
  for (const [enumType, config] of program.stateMap(stateKeys.exportedEnums)) {
    const exportName = config.name ?? enumType.name;
    if (!codeNamePattern.test(exportName)) {
      report(program, "invalid-code-name", enumType, { kind: "enum export", name: exportName });
      invalid = true;
    }

    const previous = names.get(exportName);
    if (previous) {
      report(program, "duplicate-export-name", enumType, { name: exportName });
      invalid = true;
    } else {
      names.set(exportName, enumType);
    }

    for (const name of [exportName, `${exportName}Value`]) {
    ...
    exported.push({ name: exportName, domain: config.domain, members });
  }
...
function renderApiTypes(byDomain, mode) {
  if (mode === "standalone") {
    const enums = [...byDomain.values()].flat().sort((left, right) => left.name.localeCompare(right.name, "en"));
    return [
      "// This file is generated by typespec-enum-plus-emitter. Do not edit.",
      "",
      ...enums.map(({ name, members }) =>
        `export type ${name} = ${members.map(({ value }) => renderLiteral(value)).join(" | ") || "never"}`),
      ...(enums.length === 0 ? ["export {}"] : []),
      "",
    ].join("\n");
  }
...
export async function $onEmit(context) {
  ...
  await emitFile(context.program, {
    ...
    content: renderApiTypes(byDomain, context.options["api-types-mode"] ?? "re-export"),
    newLine: "lf",
  });
